...
static void rec_render(Widget *self, RenderTree *out) {
    ...
    children[1].u.list.item_count = d->status_count;
    children[1].u.list.selected = d->selected;
    children[1].u.list.items = arena_alloc(g_session_arena, d->status_count * sizeof(ListItem));
    ...
    children[2].u.text.content = arena_strdup(g_session_arena, footer);
