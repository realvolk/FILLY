...
static char *pw_get(PWData *d, const char *k) {
    for (int i = 0; i < d->val_count; i++)
        if (strcmp(d->keys[i], k) == 0) return d->vals[i];
    return NULL;
}

static void pw_set(PWData *d, const char *k, const char *v) {
    for (int i = 0; i < d->val_count; i++) {
...
static void pw_render(Widget *self, RenderTree *out) {
    ...
    if (d->cat_count == 0) {
        ...
        children[idx].u.text.content = "No categories configured.";
        ...
        children[idx].u.list.item_count = ic;
        children[idx].u.list.selected = d->item_idx;
        children[idx].u.list.items = arena_alloc(g_session_arena, ic * sizeof(ListItem));
        for (int i = 0; i < ic; i++) {
            char *val = pw_get(d, d->item_ids[d->cat_idx][i]);
            char label[512];
    ...
    children[idx].u.text.content = d->mode == 1 ? "[Y]es  [N]o" : "Up/Down:items  Left/Right:categories  Enter:edit  F1:Build  Esc:cancel";
...
static EventResult pw_handle(Widget *self, Event *ev, Backend *backend) {
    ...
    if (d->mode == 1) {
        if (ev->code == KEY_CHAR && (ev->ch == 'y' || ev->ch == 'Y')) {
            ...
            for (int i = 0; i < d->val_count; i++)
                cJSON_AddStringToObject(obj, d->keys[i], d->vals[i]);
    ...
    switch (ev->code) {
        ...
        case KEY_ENTER:
            if (d->cat_count > 0 && d->item_idx < d->item_counts[d->cat_idx]) {
                ...
                return event_result_response((WidgetResponse){ .result = cJSON_CreateString(id), .cancelled = false });
            }
...
Widget *poweruser_factory(const WidgetRequest *req) {
    Widget *w = calloc(1, sizeof(Widget) + sizeof(PWData));
    PWData data;
    memset(&data, 0, sizeof(data));
    cJSON *title_j = cJSON_GetObjectItem(req->params, "title");
    data.title = strdup(title_j && title_j->valuestring ? title_j->valuestring : "Power User");
    ...
    if (cats && cats->type == cJSON_Array) {
        ...
        cJSON_ArrayForEach(cat, cats) {
            ...
            cJSON_ArrayForEach(item, items) {
                ...
                pw_set(&data, data.item_ids[ci][ii], data.item_values[ci][ii]);
                ii++;
            }
            ci++;
        }
    }
    widget_base_init(w, &data, sizeof(PWData), pw_render, pw_handle, pw_destroy);
    return w;
}