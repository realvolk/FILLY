...
static void fa_render(Widget *self, RenderTree *out) {
    ForgeAnvilData *d = (ForgeAnvilData *)(self + 1);
    ...
    if (d->mode == 0) {
        ...
        char buf[256]; snprintf(buf, sizeof(buf), "Execute '%s'?", d->confirm_key);
        children[idx].u.text.content = arena_strdup(g_session_arena, buf);
        children[idx].style_class = "text"; idx++;
        children[idx].type = RNODE_TEXT;
        children[idx].rect = rect_new(1, box_h - 2, box_w - 2, 1);
        children[idx].u.text.content = "[Y]es  [N]o"; children[idx].style_class = "text";
    }
    idx++;
...
static EventResult fa_handle(Widget *self, Event *ev, Backend *backend) {
    (void)backend;
    ForgeAnvilData *d = (ForgeAnvilData *)(self + 1);
    ...
    switch (ev->code) {
        ...
        case KEY_UP: d->action_idx = d->action_idx > 0 ? d->action_idx - 1 : 0; d->base.dirty = true; return event_result_handled();
        case KEY_DOWN: if (d->cat_count > 0 && d->action_idx + 1 < d->action_counts[d->cat_idx]) d->action_idx++; d->base.dirty = true; return event_result_handled();
        case KEY_LEFT: d->cat_idx = d->cat_idx > 0 ? d->cat_idx - 1 : 0; d->action_idx = 0; d->base.dirty = true; return event_result_handled();
        case KEY_RIGHT: if (d->cat_idx + 1 < d->cat_count) d->cat_idx++; d->action_idx = 0; d->base.dirty = true; return event_result_handled();
        case KEY_ENTER:
...
static void fa_destroy(Widget *self) {
    ForgeAnvilData *d = (ForgeAnvilData *)(self + 1);
    free(d->title); free(d->confirm_key);
    for (int i = 0; i < d->cat_count; i++) {
        free(d->cat_names[i]);
        for (int j = 0; j < d->action_counts[i]; j++) { free(d->action_keys[i][j]); free(d->action_descs[i][j]); }
        free(d->action_keys[i]); free(d->action_descs[i]);
    }
    free(d->cat_names); free(d->action_keys); free(d->action_descs); free(d->action_counts);
}

Widget *forge_anvil_factory(const WidgetRequest *req) {
    Widget *w = calloc(1, sizeof(Widget) + sizeof(ForgeAnvilData));
    ForgeAnvilData data;
    memset(&data, 0, sizeof(data));
    cJSON *title_j = cJSON_GetObjectItem(req->params, "title");
    data.title = strdup(title_j && title_j->valuestring ? title_j->valuestring : "ForgeLFS Anvil");
    ...
    widget_base_init(w, &data, sizeof(ForgeAnvilData), fa_render, fa_handle, fa_destroy);
    return w;
}