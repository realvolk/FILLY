...
static void prof_render(Widget *self, RenderTree *out) {
...
static void prof_destroy(Widget *w) {
...
Widget *profile_factory(const WidgetRequest *req) {
    ...
    data.title = strdup(title_j && title_j->valuestring ? title_j->valuestring : "Profile");
    ...
    data.selected = 0;
    widget_base_init(w, &data, sizeof(ProfData), prof_render, prof_handle, prof_destroy);
    return w;
}