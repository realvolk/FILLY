...
typedef struct { char *title; char **choices; int count; int selected; bool dirty; } S3Data;

static void s3_render(Widget *self, Rect area, RenderTree *out) {
    S3Data *d = (S3Data *)(self + 1);
...
static EventResult s3_handle(Widget *self, Event *ev, Backend *backend) {
    S3Data *d = (S3Data *)(self + 1);
...
static bool s3_dirty(Widget *w) { return ((S3Data *)(w + 1))->dirty; }
static void s3_clear(Widget *w) { ((S3Data *)(w + 1))->dirty = false; }
static void s3_destroy(Widget *w) { S3Data *d = (S3Data *)(w + 1); free(d->title); for (int i = 0; i < d->count; i++) free(d->choices[i]); free(d->choices); }

Widget *stage3_factory(const WidgetRequest *req) {
    Widget *w = calloc(1, sizeof(Widget) + sizeof(S3Data));
    w->vtable.render = s3_render; w->vtable.handle_event = s3_handle;
    w->vtable.is_dirty = s3_dirty; w->vtable.clear_dirty = s3_clear; w->vtable.destroy = s3_destroy;
    S3Data *d = (S3Data *)(w + 1);
    d->title = strdup(cJSON_GetObjectItem(req->params, "title")->valuestring ?: "Stage3");
