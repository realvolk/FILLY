...
static void s3_render(Widget *self, RenderTree *out) {
    ...
    c[1].u.list.item_count = d->count; c[1].u.list.selected = d->selected;
    c[1].u.list.items = arena_alloc(g_session_arena, d->count * sizeof(ListItem));
    for (int i = 0; i < d->count; i++) c[1].u.list.items[i].label = arena_strdup(g_session_arena, d->choices[i]);
    c[1].style_class = "list";
    c[2].type = RNODE_TEXT; c[2].rect = rect_new(1, box_h - 2, box_w - 2, 1);
    c[2].u.text.content = "Up/Down:move  Enter:select  Esc:cancel";
...
static void s3_destroy(Widget *w) {
    S3Data *d = (S3Data *)(w + 1);
    free(d->title);
...
Widget *stage3_factory(const WidgetRequest *req) {
    ...
    data.title = strdup(title_j && title_j->valuestring ? title_j->valuestring : "Stage3");
    ...
    data.selected = 0;
    widget_base_init(w, &data, sizeof(S3Data), s3_render, s3_handle, s3_destroy);
    return w;
}