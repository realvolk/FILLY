...
static void gauge_render(Widget *self, Rect area, RenderTree *out) {
    ...
    int box_w = 50, box_h = 6; if (box_w > area.w - 2) box_w = area.w - 2;
    int box_x = (area.w - box_w) / 2, box_y = (area.h - box_h) / 2;

    out->accessible.role = strdup("dialog");
    out->accessible.label = strdup(d->title ? d->title : "Gauge");
    ...
    children[1].type = RNODE_GAUGE; children[1].rect = rect_new(1, 2, box_w - 2, 2);
    children[1].gauge.percent = d->percent; children[1].gauge.label = strdup(d->label);
    children[1].gauge.style = textstyle_accent();
    children[1].accessible.role = strdup("progressbar"); children[1].accessible.label = strdup(d->label);

    children[2].type = RNODE_TEXT; children[2].rect = rect_new(1, box_h - 2, box_w - 2, 1);
    children[2].text.content = strdup("Any key to dismiss"); children[2].text.align = ALIGN_CENTER;
...
static void gauge_destroy(Widget *self) { GaugeData *d = (GaugeData *)(self + 1); free(d->title); free(d->label); }

Widget *gauge_widget_new(const char *title, int percent, const char *label) {
    Widget *w = calloc(1, sizeof(Widget) + sizeof(GaugeData));
    w->vtable.render = gauge_render; w->vtable.handle_event = gauge_handle_event;
    w->vtable.is_dirty = gauge_is_dirty; w->vtable.clear_dirty = gauge_clear_dirty; w->vtable.destroy = gauge_destroy;
    GaugeData *d = (GaugeData *)(w + 1); d->title = strdup(title); d->percent = percent; d->label = strdup(label); d->dirty = true;
    return w;
}