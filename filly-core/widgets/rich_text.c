...
Widget *rich_text_widget_new(const char *content) {
    ...
    w->vtable.render = rich_text_render; w->vtable.handle_event = rich_text_handle_event;
    w->vtable.is_dirty = rich_text_is_dirty; w->vtable.clear_dirty = rich_text_clear_dirty; w->vtable.destroy = rich_text_destroy;
    RichTextData *d = (RichTextData *)(w + 1); d->content = strdup(content); d->dirty = true;
    return w;
}