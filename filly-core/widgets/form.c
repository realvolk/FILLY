...
static void form_render(Widget *self, Rect area, RenderTree *out) {
    ...
    children[1].type = RNODE_FORM;
    children[1].rect = rect_new(1, 1, box_w - 2, box_h - 3);
    children[1].form.fields = malloc(d->field_count * sizeof(FormField));
    children[1].form.field_count = d->field_count;
    for (int i = 0; i < d->field_count; i++) {
        ...
        for (int j = 0; j < d->fields[i].choice_count; j++)
            children[1].form.fields[i].choices[j] = strdup(d->fields[i].choices[j]);
        children[1].form.fields[i].placeholder = strdup(d->fields[i].placeholder);
    }
    children[1].form.focused = d->focused;
    children[1].form.submit_label = strdup(d->submit_label);
    ...
    switch (ev->code) {
        ...
        case KEY_ENTER:
            if (d->focused == d->field_count) {
                ...
                for (int i = 0; i < d->field_count; i++)
                    cJSON_AddStringToObject(obj, d->fields[i].label, d->fields[i].value);
...
static void form_destroy(Widget *self) {
    FormData *d = (FormData *)(self + 1);
    free(d->title); free(d->submit_label);
    for (int i = 0; i < d->field_count; i++) {
        free(d->fields[i].label); free(d->fields[i].widget_type);
        free(d->fields[i].value); free(d->fields[i].placeholder);
        for (int j = 0; j < d->fields[i].choice_count; j++) free(d->fields[i].choices[j]);
        free(d->fields[i].choices);
    }
    free(d->fields);
}
...
Widget *form_widget_new(const char *title, FormField *fields, int field_count, const char *submit_label) {
    ...
    w->vtable.render = form_render;
    w->vtable.handle_event = form_handle_event;
    w->vtable.is_dirty = form_is_dirty;
    w->vtable.clear_dirty = form_clear_dirty;
    w->vtable.destroy = form_destroy;
    FormData *d = (FormData *)(w + 1);
    d->title = strdup(title);
    d->fields = malloc(field_count * sizeof(FormField));
    d->field_count = field_count;
    for (int i = 0; i < field_count; i++) {
        d->fields[i].label = strdup(fields[i].label);
        d->fields[i].widget_type = strdup(fields[i].widget_type);
        d->fields[i].value = strdup(fields[i].value);
        d->fields[i].choices = fields[i].choices ? malloc(fields[i].choice_count * sizeof(char *)) : NULL;
        d->fields[i].choice_count = fields[i].choice_count;
        for (int j = 0; j < fields[i].choice_count; j++)
            d->fields[i].choices[j] = strdup(fields[i].choices[j]);
        d->fields[i].placeholder = strdup(fields[i].placeholder);
    }
    d->focused = 0;
    d->submit_label = strdup(submit_label);
    d->dirty = true;
    return w;
}