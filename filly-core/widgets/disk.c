...
static Partition parse_partition(cJSON *v) {
    Partition p = {0};
    cJSON *n = cJSON_GetObjectItem(v, "number");
    if (n) p.number = n->valueint;
    cJSON *s = cJSON_GetObjectItem(v, "start");
    if (s && s->valuestring) p.start = strdup(s->valuestring);
    cJSON *e = cJSON_GetObjectItem(v, "end");
    if (e && e->valuestring) p.end = strdup(e->valuestring);
    cJSON *sz = cJSON_GetObjectItem(v, "size");
    if (sz && sz->valuestring) p.size = strdup(sz->valuestring);
    cJSON *t = cJSON_GetObjectItem(v, "type");
    if (t && t->valuestring) p.ptype = strdup(t->valuestring);
    ...
    if (f && f->type == cJSON_Array) {
        ...
        cJSON_ArrayForEach(item, f) {
            if (item->valuestring) p.flags[i++] = strdup(item->valuestring);
        }
        p.flag_count = i;
    }
    cJSON *fs = cJSON_GetObjectItem(v, "fs_signature");
    if (fs && fs->valuestring) p.fs_signature = strdup(fs->valuestring);
    return p;
}
...
static void disk_render(Widget *self, Rect area, RenderTree *out) {
    ...
    children[idx].text.content = strdup(title_buf);
    ...
    children[idx].rect = rect_new(1, 2, box_w - 2, box_h - 5);
    children[idx].list.item_count = total;
    children[idx].list.items = malloc(total * sizeof(ListItem));
    for (int i = 0; i < d->part_count; i++) {
        char label[256];
        snprintf(label, sizeof(label), "%s%3d  %8s  %-22s", i == d->selected ? ">" : " ",
                 d->partitions[i].number, d->partitions[i].size, d->partitions[i].ptype);
    ...
    if (d->selected < d->part_count && d->mode == DMODE_MAIN) {
        ...
        children[idx].text.content = strdup(detail);
    ...
    char *footer = d->readonly ? "Up/Down:move  Q:quit  Esc:cancel" :
        "Up/Down:move  N:new  D:delete  R:resize  T:type  F:flags  W:write  Q:quit  Esc:cancel";
            ...
            case KEY_CHAR:
                if (ev->ch == 'y' || ev->ch == 'Y') {
                    ...
                    for (int i = 0; i < d->part_count; i++) {
                        ...
                        for (int j = 0; j < d->partitions[i].flag_count; j++)
                            cJSON_AddItemToArray(fl, cJSON_CreateString(d->partitions[i].flags[j]));
                        cJSON_AddItemToObject(p, "flags", fl);
                        cJSON_AddItemToArray(parts, p);
                    }
                    cJSON *free = cJSON_CreateArray();
                    for (int i = 0; i < d->free_count; i++) {
                        cJSON *fs = cJSON_CreateObject();
                        cJSON_AddStringToObject(fs, "start", d->free_space[i].start);
                        cJSON_AddStringToObject(fs, "end", d->free_space[i].end);
                        cJSON_AddStringToObject(fs, "size", d->free_space[i].size);
                        cJSON_AddItemToArray(free, fs);
                    }
                    cJSON *result = cJSON_CreateObject();
                    cJSON_AddItemToObject(result, "partitions", parts);
    ...
    if (d->mode == DMODE_RESIZE_INPUT || d->mode == DMODE_NEW_PARTITION) {
        switch (ev->code) {
            ...
            case KEY_ENTER: {
                if (d->input && strlen(d->input) > 0) {
                    ...
                    if (new_bytes > 0) {
                        if (d->mode == DMODE_NEW_PARTITION) {
                            int fi = d->editing_idx;
                            FreeSpace *fs = &d->free_space[fi];
                            long long fs_start = human_to_bytes(fs->start);
                            long long fs_end = human_to_bytes(fs->end);
                            ...
                            char buf[64];
                            snprintf(buf, sizeof(buf), "%lld", fs_start);
                            d->partitions[d->part_count].start = strdup(buf);
                            snprintf(buf, sizeof(buf), "%lld", fs_start + clamped);
                            d->partitions[d->part_count].end = strdup(buf);
                            snprintf(buf, sizeof(buf), "%lld", clamped);
                            d->partitions[d->part_count].size = strdup(buf);
                            d->partitions[d->part_count].ptype = strdup("Linux filesystem");
                            d->partitions[d->part_count].flags = NULL;
                            d->partitions[d->part_count].flag_count = 0;
                            d->partitions[d->part_count].fs_signature = NULL;
                            d->part_count++;
                            long long rem = fs_end - fs_start - clamped;
                            if (rem > 0) {
                                snprintf(buf, sizeof(buf), "%lld", fs_start + clamped);
                                free(d->free_space[fi].start);
                                d->free_space[fi].start = strdup(buf);
                                snprintf(buf, sizeof(buf), "%lld", rem);
                                free(d->free_space[fi].size);
                                d->free_space[fi].size = strdup(buf);
                            } else {
                                free(d->free_space[fi].start); free(d->free_space[fi].end); free(d->free_space[fi].size);
                                memmove(&d->free_space[fi], &d->free_space[fi + 1], (d->free_count - fi - 1) * sizeof(FreeSpace));
                                d->free_count--;
                            }
                            d->selected = d->part_count - 1;
                        } else {
                            Partition *p = &d->partitions[d->editing_idx];
                            long long old_start = human_to_bytes(p->start);
                            long long old_end = human_to_bytes(p->end);
                            long long old_size = old_end - old_start;
                            if (new_bytes < old_size) {
                                long long new_end = old_start + new_bytes;
                                char buf[64];
                                snprintf(buf, sizeof(buf), "%lld", new_end);
                                free(p->end); p->end = strdup(buf);
                                snprintf(buf, sizeof(buf), "%lld", new_bytes);
                                free(p->size); p->size = strdup(buf);
                                d->free_space = realloc(d->free_space, (d->free_count + 1) * sizeof(FreeSpace));
                                snprintf(buf, sizeof(buf), "%lld", new_end);
                                d->free_space[d->free_count].start = strdup(buf);
                                snprintf(buf, sizeof(buf), "%lld", old_end);
                                d->free_space[d->free_count].end = strdup(buf);
                                snprintf(buf, sizeof(buf), "%lld", old_end - new_end);
                                d->free_space[d->free_count].size = strdup(buf);
                                d->free_count++;
                            }
                        }
                    }
                ...
                d->mode = DMODE_MAIN; d->dirty = true;
                return event_result_handled();
            }
            case KEY_BACKSPACE:
                if (d->input && strlen(d->input) > 0) d->input[strlen(d->input) - 1] = '\0';
                d->dirty = true; return event_result_handled();
            case KEY_CHAR: {
                int len = d->input ? strlen(d->input) : 0;
                d->input = realloc(d->input, len + 2);
                d->input[len] = ev->ch; d->input[len + 1] = '\0';
                d->dirty = true; return event_result_handled();
            }
        ...
        switch (ev->ch) {
            case 'n':
                if (d->selected >= d->part_count && d->selected - d->part_count < d->free_count) {
                    ...
                    d->input = strdup("");
                    d->mode = DMODE_NEW_PARTITION;
                    d->dirty = true;
                }
                return event_result_handled();
            case 'd':
                if (d->selected < d->part_count) {
                    Partition *p = &d->partitions[d->selected];
                    d->free_space = realloc(d->free_space, (d->free_count + 1) * sizeof(FreeSpace));
                    d->free_space[d->free_count].start = strdup(p->start);
                    d->free_space[d->free_count].end = strdup(p->end);
                    d->free_space[d->free_count].size = strdup(p->size);
                    d->free_count++;
                    free(p->start); free(p->end); free(p->size); free(p->ptype);
                    for (int i = 0; i < p->flag_count; i++) free(p->flags[i]);
                    free(p->flags); free(p->fs_signature);
                    memmove(&d->partitions[d->selected], &d->partitions[d->selected + 1], (d->part_count - d->selected - 1) * sizeof(Partition));
                    d->part_count--;
                    if (d->selected >= d->part_count && d->selected > 0) d->selected--;
                    d->dirty = true;
                }
                return event_result_handled();
            case 't':
                if (d->selected < d->part_count) {
                    d->editing_idx = d->selected;
                    d->type_selected = 0;
                    d->mode = DMODE_TYPE_PICKER;
                    d->dirty = true;
                }
                return event_result_handled();
            case 'f':
...
static void disk_destroy(Widget *self) {
    ...
    for (int i = 0; i < d->part_count; i++) {
        free(d->partitions[i].start); free(d->partitions[i].end);
        free(d->partitions[i].size); free(d->partitions[i].ptype);
        for (int j = 0; j < d->partitions[i].flag_count; j++) free(d->partitions[i].flags[j]);
        free(d->partitions[i].flags); free(d->partitions[i].fs_signature);
    }
    free(d->partitions);
    for (int i = 0; i < d->free_count; i++) {
        free(d->free_space[i].start); free(d->free_space[i].end); free(d->free_space[i].size);
    }
    free(d->free_space);
}
...
Widget *disk_widget_new(const char *title, const char *disk, cJSON *partitions_json, cJSON *free_space_json, bool readonly) {
    ...
    w->vtable.render = disk_render;
    w->vtable.handle_event = disk_handle_event;
    w->vtable.is_dirty = disk_is_dirty;
    w->vtable.clear_dirty = disk_clear_dirty;
    w->vtable.destroy = disk_destroy;
    ...
    d->part_count = 0;
    d->partitions = NULL;
    if (partitions_json && partitions_json->type == cJSON_Array) {
        d->part_count = cJSON_GetArraySize(partitions_json);
        d->partitions = malloc(d->part_count * sizeof(Partition));
        int i = 0;
        cJSON *item;
        cJSON_ArrayForEach(item, partitions_json) d->partitions[i++] = parse_partition(item);
    }

    d->free_count = 0;
    d->free_space = NULL;
    if (free_space_json && free_space_json->type == cJSON_Array) {
        d->free_count = cJSON_GetArraySize(free_space_json);
        d->free_space = malloc(d->free_count * sizeof(FreeSpace));
        int i = 0;
        cJSON *item;
        cJSON_ArrayForEach(item, free_space_json) {
            cJSON *s = cJSON_GetObjectItem(item, "start");
            cJSON *e = cJSON_GetObjectItem(item, "end");
            cJSON *sz = cJSON_GetObjectItem(item, "size");
            d->free_space[i].start = s && s->valuestring ? strdup(s->valuestring) : strdup("0");
            d->free_space[i].end = e && e->valuestring ? strdup(e->valuestring) : strdup("0");
            d->free_space[i].size = sz && sz->valuestring ? strdup(sz->valuestring) : strdup("0");
            i++;
        }
    }
    return w;
