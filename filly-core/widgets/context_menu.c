...
static void cm_render(Widget *self, Rect area, RenderTree *out) {
    ...
    int w = 25, h = d->count + 2;
    if (w > area.w - 4) w = area.w - 4;
    int x = (area.w - w) / 2, y = (area.h - h) / 2;

    RenderTree *children = calloc(2 + 1, sizeof(RenderTree));
    children[0].type = RNODE_LIST;
    children[0].rect = rect_new(0, 0, w - 2, h - 2);
    children[0].list.item_count = d->count;
    children[0].list.items = malloc(d->count * sizeof(ListItem));
    for (int i = 0; i < d->count; i++)
        children[0].list.items[i] = listitem_new(d->items[i]);
    children[0].list.selected = d->selected;
    children[0].list.highlight = textstyle_selected();

    children[1].type = RNODE_TEXT;
    children[1].rect = rect_new(0, h - 2, w - 2, 1);
...
static EventResult cm_handle_event(Widget *self, Event *ev, Backend *backend) {
    ...
    switch (ev->code) {
        ...
        case KEY_ENTER:
            return event_result_response((WidgetResponse){ .result = cJSON_CreateString(d->items[d->selected]), .cancelled = false, .error = NULL });
...
Widget *context_menu_widget_new(char **items, int count) {
    ...
    w->vtable.is_dirty = cm_is_dirty;
    w->vtable.clear_dirty = cm_clear_dirty;
