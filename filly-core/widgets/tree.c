...
static void tree_render(Widget *self, Rect area, RenderTree *out) {
    ...
    for (int i = 0; i < d->node_count; i++) {
        ...
        if (d->nodes[i].expanded && d->nodes[i].child_count > 0)
            for (int j = 0; j < d->nodes[i].child_count; j++) d->flat_indices[d->flat_count++] = d->node_count + j;
    }
    ...
    out->accessible.label = strdup(d->title ? d->title : "Tree");
    ...
    children[1].tree.nodes = d->nodes; children[1].tree.node_count = d->node_count;
    children[1].tree.selected_path = d->selected_path; children[1].tree.path_len = d->path_len;
    children[1].tree.highlight = textstyle_selected();
    children[1].accessible.role = strdup("tree");

    children[2].type = RNODE_TEXT; children[2].rect = rect_new(1, box_h - 2, box_w - 2, 1);
    children[2].text.content = strdup("Up/Down:move  Enter/Space:expand  Esc:dismiss");
...
static EventResult tree_handle_event(Widget *self, Event *ev, Backend *backend) {
    ...
    switch (ev->code) {
        case KEY_ESC: return event_result_response((WidgetResponse){ .result = NULL, .cancelled = false, .error = NULL });
...
Widget *tree_widget_new(const char *title, TreeNode *nodes, int node_count) {
    ...
    w->vtable.render = tree_render; w->vtable.handle_event = tree_handle_event;
    w->vtable.is_dirty = tree_is_dirty; w->vtable.clear_dirty = tree_clear_dirty; w->vtable.destroy = tree_destroy;
    TreeData *d = (TreeData *)(w + 1); d->title = strdup(title); d->nodes = nodes; d->node_count = node_count;
    d->cursor = 0; d->flat_indices = NULL; d->flat_count = 0; d->dirty = true;
    return w;
}