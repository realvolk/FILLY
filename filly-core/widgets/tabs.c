...
static void tabs_render(Widget *self, Rect area, RenderTree *out) {
    ...
    out->tabs.tab_labels = malloc(d->tab_count * sizeof(char *)); out->tabs.tab_count = d->tab_count;
    for (int i = 0; i < d->tab_count; i++) out->tabs.tab_labels[i] = strdup(d->tab_labels[i]);
    out->tabs.active = d->active;
    out->accessible.role = strdup("tablist");
    out->accessible.label = strdup(d->title ? d->title : "Tabs");
...
static void tabs_destroy(Widget *self) {
    TabsData *d = (TabsData *)(self + 1); free(d->title);
...
Widget *tabs_widget_new(const char *title, char **tab_labels, int tab_count, Widget **children, int child_count) {
    ...
    w->vtable.render = tabs_render; w->vtable.handle_event = tabs_handle_event;
    w->vtable.is_dirty = tabs_is_dirty; w->vtable.clear_dirty = tabs_clear_dirty; w->vtable.destroy = tabs_destroy;
    TabsData *d = (TabsData *)(w + 1); d->title = strdup(title);
    d->tab_labels = malloc(tab_count * sizeof(char *)); d->tab_count = tab_count;
    for (int i = 0; i < tab_count; i++) d->tab_labels[i] = strdup(tab_labels[i]);
    d->children = malloc(child_count * sizeof(Widget *)); d->child_count = child_count;
    for (int i = 0; i < child_count; i++) d->children[i] = children[i];
    d->active = 0; d->dirty = true;
    return w;
}