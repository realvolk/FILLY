...
static void table_render(Widget *self, Rect area, RenderTree *out) {
    ...
    out->accessible.label = strdup(d->title ? d->title : "Table");
    ...
    children[1].table.headers = malloc(d->header_count * sizeof(char *)); children[1].table.header_count = d->header_count;
    for (int i = 0; i < d->header_count; i++) children[1].table.headers[i] = strdup(d->headers[i]);
    children[1].table.rows = malloc(d->row_count * sizeof(char **)); children[1].table.row_count = d->row_count;
    for (int i = 0; i < d->row_count; i++) { children[1].table.rows[i] = malloc(d->header_count * sizeof(char *)); for (int j = 0; j < d->header_count; j++) children[1].table.rows[i][j] = strdup(d->rows[i][j]); }
    ...
    children[1].accessible.role = strdup("table");
...
static EventResult table_handle_event(Widget *self, Event *ev, Backend *backend) {
    ...
    switch (ev->code) {
        ...
        case KEY_ENTER: if (d->selected_row < d->row_count && d->selected_col < d->header_count) return event_result_response((WidgetResponse){ .result = cJSON_CreateString(d->rows[d->selected_row][d->selected_col]), .cancelled = false, .error = NULL }); return event_result_handled();
...
static void table_destroy(Widget *self) {
    TableData *d = (TableData *)(self + 1); free(d->title);
    for (int i = 0; i < d->header_count; i++) { free(d->headers[i]); }
    free(d->headers);
    for (int i = 0; i < d->row_count; i++) { for (int j = 0; j < d->header_count; j++) free(d->rows[i][j]); free(d->rows[i]); }
    free(d->rows);
}
...
Widget *table_widget_new(const char *title, char **headers, int header_count, char ***rows, int row_count) {
    ...
    w->vtable.render = table_render; w->vtable.handle_event = table_handle_event;
    w->vtable.is_dirty = table_is_dirty; w->vtable.clear_dirty = table_clear_dirty; w->vtable.destroy = table_destroy;
    TableData *d = (TableData *)(w + 1); d->title = strdup(title);
    d->headers = malloc(header_count * sizeof(char *)); d->header_count = header_count;
    for (int i = 0; i < header_count; i++) d->headers[i] = strdup(headers[i]);
    d->rows = malloc(row_count * sizeof(char **)); d->row_count = row_count;
    for (int i = 0; i < row_count; i++) { d->rows[i] = malloc(header_count * sizeof(char *)); for (int j = 0; j < header_count; j++) d->rows[i][j] = strdup(rows[i][j]); }
    d->selected_row = 0; d->selected_col = 0; d->dirty = true;
    return w;
}