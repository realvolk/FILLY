...
static int days_in_month(int year, int month) {
    switch (month) { case 1: case 3: case 5: case 7: case 8: case 10: case 12: return 31; case 4: case 6: case 9: case 11: return 30; case 2: return (year % 4 == 0 && (year % 100 != 0 || year % 400 == 0)) ? 29 : 28; default: return 30; }
}

static void calendar_render(Widget *self, Rect area, RenderTree *out) {
    CalendarData *d = (CalendarData *)(self + 1);
    memset(out, 0, sizeof(*out));
    int box_w = 40, box_h = 12; if (box_w > area.w - 2) box_w = area.w - 2;
    int box_x = (area.w - box_w) / 2, box_y = (area.h - box_h) / 2;

    out->accessible.role = strdup("dialog");
    out->accessible.label = strdup(d->title ? d->title : "Calendar");
    ...
    char title_buf[64]; snprintf(title_buf, sizeof(title_buf), "%s %s", d->title, months[d->month - 1]);
    children[0].text.content = strdup(title_buf); children[0].text.align = ALIGN_CENTER;
    children[0].text.style = textstyle_selected();
    children[0].accessible.role = strdup("heading"); children[0].accessible.label = strdup(title_buf);

    children[1].type = RNODE_CALENDAR; children[1].rect = rect_new(1, 1, box_w - 2, box_h - 3);
    children[1].calendar.year = d->year; children[1].calendar.month = d->month;
    children[1].calendar.selected_day = d->selected_day; children[1].calendar.highlight = textstyle_selected();
...
static EventResult calendar_handle_event(Widget *self, Event *ev, Backend *backend) {
    ...
    switch (ev->code) {
        ...
        case KEY_ENTER: { char date[16]; snprintf(date, sizeof(date), "%d-%02d-%02d", d->year, d->month, d->selected_day); return event_result_response((WidgetResponse){ .result = cJSON_CreateString(date), .cancelled = false, .error = NULL }); }
...
Widget *calendar_widget_new(const char *title) {
    ...
    w->vtable.render = calendar_render; w->vtable.handle_event = calendar_handle_event;
    w->vtable.is_dirty = calendar_is_dirty; w->vtable.clear_dirty = calendar_clear_dirty; w->vtable.destroy = calendar_destroy;
    CalendarData *d = (CalendarData *)(w + 1); d->title = strdup(title);
    time_t t = time(NULL); struct tm *tm = localtime(&t);
    d->year = tm->tm_year + 1900; d->month = tm->tm_mon + 1; d->selected_day = tm->tm_mday; d->dirty = true;
    return w;
}