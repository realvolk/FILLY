...
static void next_token(Parser *p) {
    ...
    if (p->src[p->pos] == '{') {
        p->pos++;
        p->current = make_token(TOK_LBRACE, NULL, 0);
        return;
    }
    if (p->src[p->pos] == '}') {
        p->pos++;
        p->current = make_token(TOK_RBRACE, NULL, 0);
        return;
    }
...
static bool eval_style_block(Parser *p, EvalState *s) {
    if (!match(p, TOK_STYLE)) return false;
    if (p->current.type != TOK_IDENT) return false;
    char *widget = strdup(p->current.text);
    next_token(p);
    char *child = NULL;
    if (p->current.type == TOK_IDENT) { child = strdup(p->current.text); next_token(p); }
    if (!match(p, TOK_LBRACE)) return false;
    while (p->current.type != TOK_RBRACE && p->current.type != TOK_EOF) {
        if (p->current.type != TOK_IDENT) return false;
        char *prop = strdup(p->current.text);
        next_token(p);
        if (!match(p, TOK_IS)) return false;
        char val[256];
        if (!eval_expression(p, val, sizeof(val))) return false;
        char *entry = malloc(strlen(widget) + (child ? strlen(child) + 1 : 0) + strlen(prop) + strlen(val) + 4);
        sprintf(entry, "%s|%s|%s|%s", widget, child ? child : "", prop, val);
        s->style_count++;
        s->style_widget = realloc(s->style_widget, s->style_count * sizeof(char *));
        s->style_widget[s->style_count-1] = entry;
        s->style_props = realloc(s->style_props, s->style_count * sizeof(char *));
        s->style_props[s->style_count-1] = strdup(prop);
        free(prop);
    }
    free(widget); free(child);
    match(p, TOK_RBRACE);
    return true;
}

static bool eval_keymap_block(Parser *p, EvalState *s) {
    if (!match(p, TOK_KEYMAP)) return false;
    if (p->current.type != TOK_IDENT) return false;
    char *scope = strdup(p->current.text);
    next_token(p);
    if (!match(p, TOK_LBRACE)) return false;
    while (p->current.type != TOK_RBRACE && p->current.type != TOK_EOF) {
        if (!match(p, TOK_WHEN)) return false;
        if (!match(p, TOK_KEY)) return false;
        if (!match(p, TOK_IS)) return false;
        if (p->current.type != TOK_STRING) return false;
        char *key = strdup(p->current.text);
        next_token(p);
        if (!match(p, TOK_THEN)) return false;
        if (p->current.type != TOK_STRING) return false;
        char *action = strdup(p->current.text);
        next_token(p);
        if (!match(p, TOK_END)) return false;
        char *entry = malloc(strlen(scope) + strlen(key) + strlen(action) + 4);
        sprintf(entry, "%s|%s|%s", scope, key, action);
        s->keymap_count++;
        s->keymap_bindings = realloc(s->keymap_bindings, s->keymap_count * sizeof(char *));
        s->keymap_bindings[s->keymap_count-1] = entry;
        free(key); free(action);
    }
    free(scope);
    match(p, TOK_RBRACE);
    return true;
...
FilResult *fil_eval(const char *script, const char *(*store_get)(const char *key), const char *current_value) {
    ...
    r->style_widget = s.style_widget;
    r->style_props = s.style_props;
...
void fil_result_free(FilResult *r) {
    ...
    free(r->set_keys); free(r->set_vals);
    for (int i = 0; i < r->style_count; i++) { free(r->style_widget[i]); }
