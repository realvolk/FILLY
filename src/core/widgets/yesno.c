...
static void yesno_render(Widget *self, RenderTree *out) {
    ...
    if (d->message && strlen(d->message)) {
        children[idx].type = RNODE_TEXT;
        children[idx].rect = rect_new(1, 2, box_w - 2, 2);
        children[idx].u.text.content = arena_strdup(g_session_arena, d->message);
        children[idx].style_class = "text";
        idx++;
    }
    children[idx].type = RNODE_TEXT;
    children[idx].rect = rect_new((box_w - 20) / 2, 5, 8, 1);
    children[idx].u.text.content = "[ Yes ]";
    children[idx].style_class = "text";
    children[idx].state = d->selected_yes ? "selected" : "normal";
    idx++;
    children[idx].type = RNODE_TEXT;
    children[idx].rect = rect_new((box_w - 20) / 2 + 10, 5, 8, 1);
    children[idx].u.text.content = "[ No ]";
    children[idx].style_class = "text";
    children[idx].state = d->selected_yes ? "normal" : "selected";
    idx++;
    children[idx].type = RNODE_TEXT;
    children[idx].rect = rect_new(1, box_h - 2, box_w - 2, 1);
    children[idx].u.text.content = "h/l:choose  Enter:confirm  y/n:quick  Esc:cancel";
    children[idx].style_class = "text";
...
static EventResult yesno_handle_event(Widget *self, Event *ev, Backend *backend) {
    ...
    switch (ev->code) {
        ...
        case KEY_CHAR:
            if (ev->ch == 'y' || ev->ch == 'Y') return event_result_response((WidgetResponse){ .result = cJSON_CreateBool(1), .cancelled = false });
            if (ev->ch == 'n' || ev->ch == 'N') return event_result_response((WidgetResponse){ .result = cJSON_CreateBool(0), .cancelled = false });
