...
static HubItem *hub_get_item(HubData *d) {
    ...
    for (int i = 0; i < cat->item_count; i++) {
        ...
        if (vi == d->item_idx) return &cat->items[i];
        vi++;
    }
    return NULL;
}
...
static void hub_render(Widget *self, RenderTree *out) {
    ...
    if (d->mode == HUB_EDITING && d->sub_widget) {
        d->sub_widget->vtable.render(d->sub_widget, out);
        return;
    }
    ...
    if (d->mode == HUB_CONFIRM_PROCEED || d->mode == HUB_CONFIRM_QUIT) {
        ...
        children[1].u.text.content = d->mode == HUB_CONFIRM_PROCEED ? "Proceed?" : "Quit without saving?";
        children[1].style_class = "text";
        children[2].type = RNODE_TEXT;
        children[2].rect = rect_new(1, 4, box_w - 2, 1);
        children[2].u.text.content = "[Y]es  [N]o";
        children[2].style_class = "text";
        out->type = RNODE_CONTAINER;
        out->rect = rect_new((area.w - box_w) / 2, (area.h - 7) / 2, box_w, 7);
    ...
    for (int i = 0; i < d->cat_count; i++) {
        char l[256];
        snprintf(l, sizeof(l), "%s %s", i == d->cat_idx ? ">" : " ", d->categories[i].label);
        children[idx].u.list.items[i].label = arena_strdup(g_session_arena, l);
    }
    children[idx].style_class = "list";
    idx++;

    HubCategory *cat = NULL;
    int vis = 0;
    if (d->cat_count > 0) {
        cat = &d->categories[d->cat_idx];
        vis = hub_visible_count(d);
    }
    children[idx].type = RNODE_LIST;
    ...
    if (cat) {
        ...
        char l[512];
        snprintf(l, sizeof(l), "%s %s: %s", vi == d->item_idx ? ">" : " ", item->label, disp);
        children[idx].u.list.items[vi].label = arena_strdup(g_session_arena, l);
        vi++;
        }
    }
    children[idx].style_class = "list";
    ...
    for (int i = 0; i < d->action_count; i++) {
        char b[32];
        snprintf(b, sizeof(b), "F%d:%s  ", i + 1, d->actions[i]);
        strcat(footer, b);
    }
...
static EventResult hub_handle_event(Widget *self, Event *ev, Backend *backend) {
    ...
    if (d->mode == HUB_CONFIRM_PROCEED) {
        if (ev->code == KEY_CHAR && (ev->ch == 'y' || ev->ch == 'Y')) {
            ...
            for (int i = 0; i < d->val_count; i++)
                cJSON_AddStringToObject(r, d->keys[i], d->vals[i]);
            d->base.accepts_text_input = false;
        ...
        if (r.type == EVENT_RESULT_RESPONSE) {
            ...
            if (item && !r.response.cancelled && r.response.result) {
                if (r.response.result->valuestring)
                    ...
                    char *j = cJSON_PrintUnformatted(r.response.result);
                    hub_set(d, item->id, j);
                    free(j);
                }
            }
    ...
    switch (ev->code) {
        case KEY_ESC:
            d->mode = HUB_CONFIRM_QUIT;
            d->base.dirty = true;
            return event_result_handled();
        case KEY_UP:
            if (d->item_idx > 0) d->item_idx--;
            d->base.dirty = true;
            return event_result_handled();
        case KEY_DOWN:
            if (d->item_idx + 1 < hub_visible_count(d)) d->item_idx++;
            d->base.dirty = true;
            return event_result_handled();
        case KEY_LEFT:
            d->cat_idx = d->cat_idx > 0 ? d->cat_idx - 1 : d->cat_count - 1;
            d->item_idx = 0;
            d->base.dirty = true;
            return event_result_handled();
        case KEY_RIGHT:
        case KEY_TAB:
            d->cat_idx = d->cat_idx + 1 < d->cat_count ? d->cat_idx + 1 : 0;
            d->item_idx = 0;
            d->base.dirty = true;
            return event_result_handled();
        case KEY_ENTER: {
            ...
            char *cur = hub_get(d, item->id);
            if (!cur) cur = item->value;
            ...
            if (item->message && strlen(item->message))
                cJSON_AddStringToObject(req.params, "message", item->message);
            if (cur && strlen(cur))
                cJSON_AddStringToObject(req.params, "default", cur);
            if (item->placeholder && strlen(item->placeholder))
                cJSON_AddStringToObject(req.params, "placeholder", item->placeholder);
        ...
        default: {
            if (ev->code >= KEY_F1 && ev->code <= KEY_F12) {
                ...
                if (f < d->action_count) {
                    ...
                    return event_result_response((WidgetResponse){
                        .result = cJSON_CreateString(d->actions[f]), .cancelled = false
                    });
                }
            }
...
static void hub_destroy(Widget *self) {
    ...
    free(d->title);
    for (int i = 0; i < d->cat_count; i++) {
        free(d->categories[i].id);
        free(d->categories[i].label);
        free(d->categories[i].summary_template);
        for (int j = 0; j < d->categories[i].item_count; j++) {
            ...
            free(item->message);
            free(item->display);
            for (int k = 0; k < item->visible_if.count; k++) {
                free(item->visible_if.keys[k]);
                free(item->visible_if.vals[k]);
            }
            free(item->visible_if.keys);
            free(item->visible_if.vals);
        }
...
Widget *hub_widget_new(const char *title, cJSON *categories_json, cJSON *actions_json) {
    ...
    if (categories_json && cJSON_IsArray(categories_json)) {
        data.cat_count = cJSON_GetArraySize(categories_json);
        data.categories = calloc(data.cat_count, sizeof(HubCategory));
        int ci = 0;
        cJSON *cv;
        cJSON_ArrayForEach(cv, categories_json) {
            cJSON *cid = cJSON_GetObjectItem(cv, "id");
            cJSON *cl = cJSON_GetObjectItem(cv, "label");
            cJSON *ct = cJSON_GetObjectItem(cv, "summary_template");
            data.categories[ci].id = cid && cid->valuestring ? strdup(cid->valuestring) : strdup("");
            data.categories[ci].label = cl && cl->valuestring ? strdup(cl->valuestring) : strdup("");
            data.categories[ci].summary_template = ct && ct->valuestring ? strdup(ct->valuestring) : strdup("");

            cJSON *items = cJSON_GetObjectItem(cv, "items");
            data.categories[ci].item_count = items ? cJSON_GetArraySize(items) : 0;
            ...
            cJSON_ArrayForEach(iv, items) {
                ...
                cJSON *iid = cJSON_GetObjectItem(iv, "id");
                item->id = iid && iid->valuestring ? strdup(iid->valuestring) : strdup("");
                cJSON *il = cJSON_GetObjectItem(iv, "label");
                item->label = il && il->valuestring ? strdup(il->valuestring) : strdup("");
                cJSON *ival = cJSON_GetObjectItem(iv, "value");
                item->value = ival && ival->valuestring ? strdup(ival->valuestring) : strdup("");
                cJSON *iw = cJSON_GetObjectItem(iv, "widget");
                item->widget = iw && iw->valuestring ? strdup(iw->valuestring) : strdup("menu");
                cJSON *ich = cJSON_GetObjectItem(iv, "choices");
                ...
                cJSON *ip = cJSON_GetObjectItem(iv, "placeholder");
                item->placeholder = ip && ip->valuestring ? strdup(ip->valuestring) : strdup("");
                cJSON *im = cJSON_GetObjectItem(iv, "message");
                item->message = im && im->valuestring ? strdup(im->valuestring) : strdup("");
                cJSON *idisp = cJSON_GetObjectItem(iv, "display");
                item->display = idisp && idisp->valuestring ? strdup(idisp->valuestring) : NULL;
                cJSON *dp = cJSON_GetObjectItem(iv, "disk_picker");
                item->disk_picker = dp && dp->valueint;
                cJSON *vi = cJSON_GetObjectItem(iv, "visible_if");
                item->visible_if.keys = NULL;
                item->visible_if.vals = NULL;
                item->visible_if.count = 0;
    ...
    data.action_count = 0;
    data.actions = NULL;
    if (actions_json && cJSON_IsArray(actions_json)) {
        data.action_count = cJSON_GetArraySize(actions_json);
        data.actions = malloc(data.action_count * sizeof(char *));
        for (int i = 0; i < data.action_count; i++)
            data.actions[i] = strdup(cJSON_GetArrayItem(actions_json, i)->valuestring);
    }
