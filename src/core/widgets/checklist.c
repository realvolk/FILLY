...
static void checklist_render(Widget *self, RenderTree *out) {
    ...
    if (d->title && strlen(d->title)) {
        ...
        children[idx].rect = rect_new(ch, 0, box_w - 2*ch, ch);
    ...
    if (d->message && strlen(d->message)) {
        children[idx].type = RNODE_TEXT;
        children[idx].rect = rect_new(ch, ch, box_w - 2*ch, 2*ch);
        children[idx].u.text.content = arena_strdup(g_session_arena, d->message);
        children[idx].style_class = "text";
        idx++;
        list_y = 3 * ch;
    }
    int list_h = box_h - list_y - 2*ch;
    children[idx].type = RNODE_LIST;
    children[idx].rect = rect_new(ch, list_y, box_w - 2*ch, list_h > 0 ? list_h : ch);
    ...
    children[idx].type = RNODE_TEXT;
    children[idx].rect = rect_new(ch, box_h - 2*ch, box_w - 2*ch, ch);
    char footer[256];
