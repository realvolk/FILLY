...
static void text_editor_render(Widget *self, RenderTree *out) {
...
static EventResult text_editor_handle_event(Widget *self, Event *ev, Backend *backend) {
    ...
    switch (ev->code) {
        case KEY_ESC: {
            ...
            cJSON *r = cJSON_CreateString(full);
            free(full);
    ...
    if (d->line_count == 0 && content) {
        ...
        while (*p) {
            if (*p == '\n') {
                ...
                memcpy(d->lines[d->line_count], s, len);
