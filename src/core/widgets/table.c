...
static void table_render(Widget *self, RenderTree *out) {
    ...
    children[1].u.table.header_count = d->header_count;
    children[1].u.table.row_count = d->row_count;
    children[1].u.table.selected_row = d->selected_row;
    children[1].u.table.selected_col = d->selected_col;
    children[1].u.table.headers = arena_alloc(g_session_arena, d->header_count * sizeof(char *));
    for (int i = 0; i < d->header_count; i++) {
        children[1].u.table.headers[i] = arena_strdup(g_session_arena, d->headers[i]);
    }
    children[1].u.table.rows = arena_alloc(g_session_arena, d->row_count * sizeof(char **));
    for (int i = 0; i < d->row_count; i++) {
        children[1].u.table.rows[i] = arena_alloc(g_session_arena, d->header_count * sizeof(char *));
        for (int j = 0; j < d->header_count; j++) {
            children[1].u.table.rows[i][j] = arena_strdup(g_session_arena, d->rows[i][j]);
        }
    }
    ...
    children[2].u.text.content = "Up/Down:row  Left/Right:col  Enter:select  Esc:cancel";
...
static EventResult table_handle_event(Widget *self, Event *ev, Backend *backend) {
    ...
    switch (ev->code) {
        ...
        case KEY_UP:
            if (d->selected_row > 0) d->selected_row--;
            d->base.dirty = true;
            return event_result_handled();
        case KEY_DOWN:
            if (d->selected_row + 1 < d->row_count) d->selected_row++;
            d->base.dirty = true;
            return event_result_handled();
        case KEY_LEFT:
            if (d->selected_col > 0) d->selected_col--;
            d->base.dirty = true;
            return event_result_handled();
        case KEY_RIGHT:
            if (d->selected_col + 1 < d->header_count) d->selected_col++;
            d->base.dirty = true;
            return event_result_handled();
        case KEY_ENTER:
            if (d->selected_row < d->row_count && d->selected_col < d->header_count) {
                return event_result_response((WidgetResponse){
                    .result = cJSON_CreateString(d->rows[d->selected_row][d->selected_col]),
                    .cancelled = false
                });
            }
            return event_result_handled();
...
Widget *table_widget_new(const char *title, char **headers, int header_count, char ***rows, int row_count) {
    ...
    for (int i = 0; i < row_count; i++) {
        data.rows[i] = malloc(header_count * sizeof(char *));
        for (int j = 0; j < header_count; j++) data.rows[i][j] = strdup(rows[i][j]);
    }
