...
static void tree_render(Widget *self, RenderTree *out) {
    TreeData *d = (TreeData *)(self + 1);
    WidgetBase *base = (WidgetBase *)(self + 1);
    Rect area = base->render_area;
    d->flat_indices = realloc(d->flat_indices, (d->node_count*10) * sizeof(int));
    d->flat_count = 0;
    ...
    children[1].type = RNODE_TREE;
    children[1].rect = rect_new(1, 1, box_w - 2, box_h - 3);
    children[1].u.tree.nodes = d->nodes;
    children[1].u.tree.node_count = d->node_count;
    children[1].style_class = "tree";
    children[2].type = RNODE_TEXT;
    children[2].rect = rect_new(1, box_h - 2, box_w - 2, 1);
    children[2].u.text.content = "Up/Down:move  Enter/Space:expand  Esc:dismiss";
