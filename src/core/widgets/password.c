...
static void password_render(Widget *self, RenderTree *out) {
    PasswordData *d = (PasswordData *)(self + 1);
    WidgetBase *base = (WidgetBase *)(self + 1);
    Rect area = base->render_area;
    memset(out, 0, sizeof(*out));
    out->style_class = "container";
    out->accessible.role = "password";
    out->accessible.label = d->title ? d->title : "Password";
    ...
    children[idx].u.input.text = arena_strdup(g_session_arena, d->text);
    children[idx].u.input.placeholder = arena_strdup(g_session_arena, d->placeholder ? d->placeholder : "");
    children[idx].u.input.masked = true;
