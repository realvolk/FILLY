...
static void tabs_render(Widget *self, RenderTree *out) {
    ...
    for (int i = 0; i < d->tab_count; i++)
        out->u.tabs.tab_labels[i] = arena_strdup(g_session_arena, d->tab_labels[i]);
    out->u.tabs.active = d->active;
    out->style_class = "tabs";
    if (d->active < d->child_count) {
        out->u.tabs.child = arena_alloc(g_session_arena, sizeof(RenderTree));
        d->children[d->active]->vtable.render(d->children[d->active], out->u.tabs.child);
    }
}
...
static EventResult tabs_handle_event(Widget *self, Event *ev, Backend *backend) {
    ...
    if (ev->type == EVENT_KEY) {
        if (ev->code == KEY_LEFT && d->active > 0) { d->active--; d->base.dirty = true; return event_result_handled(); }
        if (ev->code == KEY_RIGHT && d->active + 1 < d->tab_count) { d->active++; d->base.dirty = true; return event_result_handled(); }
    }
    if (d->active < d->child_count)
...
Widget *tabs_widget_new(const char *title, char **tab_labels, int tab_count, Widget **children, int child_count) {
    ...
    for (int i = 0; i < child_count; i++) data.children[i] = children[i];
