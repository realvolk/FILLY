...
static void mr_render(Widget *self, RenderTree *out) {
    ...
    if (d->mode == MR_SAVE_INPUT && d->save_path) {
        ...
        children[3].u.text.content = arena_strdup(g_session_arena, buf);
    } else {
        children[3].u.text.content = arena_strdup(g_session_arena, "");
    }
